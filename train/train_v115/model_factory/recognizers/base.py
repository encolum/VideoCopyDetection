...
def build_module(cfg):
    """
    Khởi tạo module từ cấu hình.
    
    Args:
        cfg (dict): Cấu hình module, yêu cầu phải có khóa 'type'.
    
    Returns:
        nn.Module: Module được khởi tạo.
    """
    module_type = cfg.pop('type', None)
    if not module_type:
        raise ValueError("Config must include 'type' to specify the module type.")
    
    if module_type not in MODULE_MAP:
        raise ValueError(f"Module type '{module_type}' không được hỗ trợ.")
...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if cls_head is not None:
            self.cls_head = build_module(backbone)
        ...
        if custom_backbones:
            for key, cstmBbone in custom_backbones.items():
                setattr(self, key, build_module(cstmBbone))
                self._sub_components.append(key)

        if custom_heads:
            for key, cstmHd in custom_heads.items():
                setattr(self, key, build_module(cstmHd))
                self._sub_components.append(key)
