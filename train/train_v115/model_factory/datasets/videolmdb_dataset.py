...
import lmdb
from model_factory.datasets.transforms_utils import build_transforms
from model_factory.transforms import OverlayEmoji, OverlayText, CropAndPad, \
...
class VideoLmdbDataSet(torch.utils.data.Dataset):
    def __init__(
            self, vids_path, meta_path, preprocess, lmdb_path, lmdb_size=1e9, width=224,
            arg_lmdb_path="",
        ...
        self.hard_pipelines = np.array([
            A.Compose([
                ...
                A.RandomSnow(p=0.1),
                OverlayText(always_apply=False, p=0.1),
                OverlayEmoji(always_apply=False, p=0.1),
                # fix   
                RandomCompose([
                    A.OneOf([
                        CropAndPad(p=1),
                    ...
                    A.OneOf([
                        ...
                        A.RandomBrightnessContrast(brightness_limit=(-0.2, 0.1), p=1),
                        A.ToGray(p=1),
                        A.HueSaturationValue(p=1),
                        A.RandomBrightnessContrast(p=1)
                    ], p=0.8),
                    RandomStackImages(self.arg_lmdb_path, self.lmdb_size, self.width, p=0.1),
                    RandomOverlayImages(self.arg_lmdb_path, self.lmdb_size, self.width,always_apply=False, p=0.1),
                    RandomOverlayCorners(always_apply=False,p=0.1),
...
class LabelVideoLmdbDataSet(VideoLmdbDataSet):
    ...
    def __init__(
            self, vids_path, meta_path, preprocess, lmdb_path, lmdb_size=5e9, width=224,
            arg_lmdb_path="",
    ...
    def _sample_pair(self, idx):
        ...
        try:
            ...
            print(f"Error encountered while sampling pair {idx}: {e}")
            return self._sample_pair(np.random.randint(0, len(self)))
