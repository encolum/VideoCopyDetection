...
class PairWiseFeatZipDataSet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        info = self.sample(index, worker_id)

        return info

    def _read_feats(self, vid, worker_id):
        handle = self.handles[0] if worker_id is None else self.handles[worker_id]
        video_feature = np.load(io.BytesIO(handle.read(vid)), allow_pickle=True).astype(np.float32)
