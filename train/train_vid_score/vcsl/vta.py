# Tệp này triển khai các thuật toán căn chỉnh thời gian video (VTA) khác nhau để phát hiện các đoạn video gần giống nhau
# Nó chứa các triển khai của các thuật toán DTW, DP, TN và HV, cũng như các hàm tiện ích để tính toán độ tương đồng
...
# Tính độ tương đồng Chamfer giữa hai tập vector sử dụng CPU
...
# Tính độ tương đồng Chamfer giữa hai tập vector sử dụng GPU
...
# Chuẩn hóa điểm số tương đồng đến một khoảng cụ thể
...
# Tính bản đồ tương đồng giữa hai tập vector sử dụng CPU
def sim_map_cpu(qid, rid, q: np.ndarray, r: np.ndarray, normalize_input=False, similarity_type="cos"):
    ...
    if similarity_type == "cos":
        ...
        raise ValueError(f"Phương pháp không xác định {similarity_type}")


# Tính bản đồ tương đồng giữa hai tập vector sử dụng GPU
...
# Lớp để tính toán bản đồ tương đồng video
...
# Tính Intersection over Union (IoU) giữa các hộp giới hạn
def iou(bbox, gt):
    """
    :param bbox: (n, 4)
    :param gt: (m, 4)
...
# Tìm các chuỗi số không trong một mảng
def zero_runs(a):
    # Tạo một mảng là 1 khi a là 0, và thêm 0 vào mỗi đầu.
    iszero = np.concatenate(([0], np.equal(a, 0).view(np.int8), [0]))
    absdiff = np.abs(np.diff(iszero))
    # Chuỗi bắt đầu và kết thúc khi absdiff là 1.
    ranges = np.where(absdiff == 1)[0].reshape(-1, 2)
    return ranges


# Cắt một đường dẫn thành các đoạn dựa trên ngưỡng đường chéo
...
# Thuật toán Dynamic Time Warping (DTW) cho căn chỉnh thời gian
def dtw(sim_matrix: np.ndarray, discontinue=3, min_sim=0.2, min_length=5, max_iou=0.3):
    # CHÚ Ý: sim_matrix biểu diễn độ tương đồng, chúng ta cần một ma trận khoảng cách
    path, sim_score = dtw_path_from_metric(1 - sim_matrix, metric="precomputed")
    path = np.array(path)

    # loại bỏ các đường ngang và dọc
...
# Tìm đường đi tối ưu trong ma trận quy hoạch động
...
# Hàm được tăng tốc bởi Numba để tính toán ma trận quy hoạch động
@njit()
...
# Thuật toán Dynamic Programming (DP) cho căn chỉnh thời gian
def dp(sim_matrix: np.ndarray, discontinue=3, min_sim=1, ave_sim=1.3, min_length=5, diagonal_thres=30):
    # triển khai mPDP từ Pattern-Based Near-Duplicate Video Retrieval and Localization on Web-Scale Videos
    # thay đổi tỷ lệ để làm cho điểm số cosine-similarity không âm
    sim_matrix += 1
...
# Thuật toán Temporal Network (TN) cho căn chỉnh thời gian
def tn(sims: np.ndarray,
       tn_max_step=10, tn_top_k=5, max_path=10,
       min_sim=0.2, min_length=5, max_iou=0.3):
    # từ "Scalable detection of partial near-duplicate videos by visual-temporal consistency"
    infringe_box_list = []
    path = 0
    node_pair2id = {}
    node_pair2id[(-1, -1)] = 0
    ...
    # lấy giá trị và chỉ số top-k, hình dạng (Q_LEN, top_k)
    ...
    # tạo đồ thị bằng cách thêm cạnh
    for q_i in range(sims.shape[0]):
        r_i = topk_indices[q_i]

        intermediate_rs = np.empty((0,), dtype=np.int32)
        # triển khai Ràng buộc C1 bằng cách giới hạn phạm vi kết thúc
        for q_j in range(q_i + 1, min(sims.shape[0], q_i + tn_max_step)):
            r_j = topk_indices[q_j]  # hình dạng (top_k, )

            r_diff = r_j[:, None] - r_i  # đích - nguồn, hình dạng (top_k, top_k)
            ...
            if len(intermediate_rs) == 0:
                ...
                # "dấu bằng" trong C3 trong bài báo là sai vì nó mâu thuẫn với C2
                cond1 = intermediate_rs[None, :] > r_i[:, None]
                cond2 = intermediate_rs[None, :] < r_j[:, None]
                C3 = np.sum(cond2[:, None, :] & cond1, axis=-1) == 0

            # Ràng buộc C4
            s_j = topk_sims[q_j]  # hình dạng (top_k, )
            s_j = np.repeat(s_j.reshape(-1, 1), r_diff.shape[1], axis=1)  # hình dạng (top_k, top_k)
            C4 = s_j >= min_sim

            val_rows, val_cols = np.where(C2 & C3 & C4)
            val_sims = s_j[val_rows, val_cols]
            # cập nhật intermediate_rs
    ...
    while True:
        ...
        if 0 in longest_path:
            longest_path.remove(0)  # loại bỏ nút nguồn
        if node_num - 1 in longest_path:
            longest_path.remove(node_num - 1)  # loại bỏ nút đích
...
# Thuật toán Histogram Voting (HV) cho căn chỉnh thời gian
def hv(sims: np.ndarray,
    ...
    ## bước 1: loại bỏ tất cả các cặp thấp hơn min_sim
    sims[sims < min_sim] = 0.

    ## bước 2: tính toán histogram time_bins
    ...
    ## bước 3: tinh chỉnh các kết quả cuối cùng
    ...
    """
    ## bước 4: xuất ra infringe_box_list cuối cùng
    for sigma, sum in sorted_sigma_hists:
...
# Hàm wrapper để xử lý ngoại lệ trong đa xử lý
def func_wrapper_with_exception(rid, item, func):
    try:
        return rid, func(item)
    except Exception as e:
        logger.exception("Không thể chạy với rid {}", rid)
        raise RuntimeError(f"Không thể chạy với dữ liệu có {rid}") from e


# Lớp cơ sở cho các mô hình VTA
...
# Factory function to build VTA models
